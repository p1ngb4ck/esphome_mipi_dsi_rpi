...
void MIPI_DSI_RPI::setup() {
  ...
  dsi_write(0x0210, 0x00000007);  // DSI_LANEENABLE
...
void MIPI_DSI_RPI::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr, display::ColorOrder order,
...
void MIPI_DSI_RPI::write_to_display_(int x_start, int y_start, int w, int h, const uint8_t *ptr, int x_offset, int y_offset,
                                 int x_pad) {
...
bool MIPI_DSI_RPI::check_buffer_() {
  if (this->is_failed())
    return false;
...
void MIPI_DSI_RPI::draw_pixel_at(int x, int y, Color color) {
...
void MIPI_DSI_RPI::fill(Color color) {
  if (!this->check_buffer_())
    return;
  switch (this->color_depth_) {
    case display::COLOR_BITNESS_565: {
...
void MIPI_DSI_RPI::dump_config() {
  ESP_LOGCONFIG(TAG,
                "MIPI_DSI_RPI RGB LCD"
                "\n  Model: %s"
                "\n  Width: %u"
                "\n  Height: %u"
                "\n  Mirror X: %s"
